...
export class BYUObject {
    ...
    constructor(numVertices: number, numTriangles: number, vertices: number[][] = [], indexes: number[][] = []) {
        this.numVertices = numVertices;
        this.numTriangles = numTriangles;
        this.vertices = vertices;
        this.indexes = indexes;
        this.minX = vertices ? this.getMin(Coordinate.X, vertices) : 0;
        this.maxX = vertices ? this.getMax(Coordinate.X, vertices) : Number.MAX_SAFE_INTEGER;
        this.minY = vertices ? this.getMin(Coordinate.Y, vertices) : 0;
        this.maxY = vertices ? this.getMax(Coordinate.Y, vertices) : Number.MAX_SAFE_INTEGER;
        this.minZ = vertices ? this.getMin(Coordinate.Z, vertices) : 0;
        this.maxZ = vertices ? this.getMax(Coordinate.Z, vertices) : Number.MAX_SAFE_INTEGER;
    }
